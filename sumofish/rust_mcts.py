...
The default core as of 2026-07-31. `CHESSGPU_CORE=python` opts back into the
pure-Python `sumofish.mcts.MCTS`, so a rollback is still just an environment
variable, and the two can still be A/B'd directly -- Python was never deleted,
it just stopped being what an unset `CHESSGPU_CORE` silently falls back to.
...
`sumofish.mcts`, verified over 40 positions x 800 simulations at five batch sizes,
...
class RustMCTS:
    """Duck-types the parts of `sumofish.mcts.MCTS` that `search_engine` uses.
...
def select_mcts_class():
    """`RustMCTS` by default; `CHESSGPU_CORE=python` opts back into the
    pre-port Python search.

    Flipped 2026-07-31. Was defaulted to Python with Rust opt-in "until it
    has played rated games" -- it has, continuously, since 2026-07-30 (see
    ...
    choice = os.environ.get("CHESSGPU_CORE", "rust").strip().lower()
    if choice == "python":
        return MCTS, "python"
    if choice not in ("rust", ""):
        raise ValueError(
            f"CHESSGPU_CORE={choice!r} is not 'rust' or 'python'. Failing rather "
            f"than falling back, because a typo that silently selects the wrong "
