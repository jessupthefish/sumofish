...
Source of truth read in full: `/home/nomad/dev/active/sumofish/scripts/watch.py`, `/home/nomad/dev/active/sumofish/scripts/dash/{panels,widgets,theme,ink,board,sixel,state,sources,fusion}.py`, `/home/nomad/dev/active/sumofish/scripts/dash/make_sprites.py`, `/home/nomad/dev/active/sumofish/tests/verify_layout.py`, `/home/nomad/dev/active/sumofish/CLAUDE.md`, and the telemetry producer `/home/nomad/dev/active/sumofish/sumofish/engines/search_engine.py`.
...
*Last row — units and API budget:* for each unit in `state.units` (insertion order; sources tracks `sumofish-bot`, `sumofish-train`):
- `"●"` in `GOOD` iff `active == "active"` **and** `sub == "running"`, else `BAD`.
- `" {unit with 'sumofish-' stripped} "` in `DIM`.
...
`/home/nomad/dev/active/sumofish/scripts/dash/theme.py`. **It is gruvbox dark for all UI chrome**, with three custom families layered on: a board palette, a piece-ink palette, and an evaluation-gauge palette that is explicitly *not* gruvbox because gruvbox's own values failed a measured contrast requirement.
...
## B. WIDGETS — `/home/nomad/dev/active/sumofish/scripts/dash/widgets.py`
...
### C.1 Text / half-block renderer — `/home/nomad/dev/active/sumofish/scripts/dash/board.py`
...
### C.2 Sixel renderer — `/home/nomad/dev/active/sumofish/scripts/dash/sixel.py`
...
Startup requires a token: `LICHESS_BOT_TOKEN` from the environment, else the first `LICHESS_BOT_TOKEN=` line of `~/.config/sumofish/bot.env`; if neither, exit with `"LICHESS_BOT_TOKEN not set (see ~/.config/sumofish/bot.env)"`. **This applies to `--demo` too** — the fixture path still requires a token today. The token is used for exactly two endpoints (`/api/account`, `/api/account/playing`) and is never printed.
...
| `units` | both `sumofish-bot` and `sumofish-train` active/running/0 restarts |
...
Geometry invariants gated by `/home/nomad/dev/active/sumofish/tests/verify_layout.py` (no torch, no lichess, no terminal — `Caps` is constructed directly, at cell 8.0 × 14.9, across sizes 160×96, 150×44, 160×48, 200×60, 320×90, 120×40, 90×30, 80×24):
...
1. **Explicit claim.** The engine stamps `pid` and the lichess `game` id on every record (`sumofish/telemetry.py`), the id arriving over UCI as `setoption name GameId` from lichess-bot's `extra_game_handlers` hook. `_claims(rec, gid)` returns `True`/`False`/`None`. `True` → accept unconditionally, no history to satisfy. `False` → drop.
